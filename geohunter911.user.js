...
(() => {
  ...
  const centroid = async q => cache.cen[q] ??=
    (await jFetch(GEO(`search.php?q=${encodeURIComponent(q)}&format=json&limit=1`)))?.[0] || null;
  ...
  function buildHUD(){
    ...
    ['+','−'].forEach((s,i)=>{
      ...
      b.onclick=()=>{zoom=Math.min(20,Math.max(1,zoom+(s==='+'?1:-1)));ui.map.src=MAP(shown.lat,shown.lon,zoom);};
      wrap.appendChild(b);
    });
  ...
  XMLHttpRequest.prototype.open=new Proxy(XMLHttpRequest.prototype.open,{
    apply(o,xhr,[m,url]){if(m==='POST'&&url.includes('MapsJsInternalService')){
      xhr.addEventListener('load',()=>{
        ...
        if(m){const[lat,lon]=m[0].split(',').map(Number);latest.lat=lat;latest.lon=lon;latest.ready=true;}
      });}
      return o.apply(xhr,[m,url]);
    }
  });
  ...
  document.addEventListener('keydown',async e=>{
    ...
    if(country){
      ...
      if(cc&&city){
        ...
        if(ci){cityInCountry=` (${dir(+cc.lat,+cc.lon,+ci.lat,+ci.lon,0.05)})`;
